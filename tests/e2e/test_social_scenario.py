...
def test_comment_ownership_and_unique_like_state(
    *,
    client: TestClient,
) -> None:
    _register(client=client, email="alice@example.com", name="Alice")
    ...
    created = client.post(
        f"/api/v1/http/posts/{post['id']}/comments",
        headers=alice_auth,
        json={"post_id": f"{post['id']}","content": "A literal 100% _useful_ first comment"},
    )
    assert created.status_code == 201, created.text
    comment = created.json()

    forbidden_update = client.put(
        f"/api/v1/http/posts/{comment['id']}",
        headers=bob_auth,
        json={"content": "stolen"},
    )
    forbidden_delete = client.delete(
    ...
    liked = client.post(
        f"/api/v1/http/comments/{comment['id']}/likes/toggle",
        headers=bob_auth,
    )
    unliked = client.post(
        f"/api/v1/http/comments/{comment['id']}/likes/toggle",
