...
from application.common.exceptions import (
    ...
    ConflictError,
    ForbiddenError,
    NotFoundError, MediaTooLargeError, UnsupportedMediaTypeError,
)
...
def install_application_error_handlers(*, app: FastAPI) -> None:
    ...
    async def media_too_large(_, __) -> JSONResponse:
        return JSONResponse(
            status_code=413,
            content={"detail": "Media file is too large"},
        )


    @app.exception_handler(UnsupportedMediaTypeError)
    async def unsupported_media(_, __) -> JSONResponse:
        return JSONResponse(
            status_code=415,
